...
import { Send, Smile, Paperclip, X, ChevronDown, Home, Briefcase, Wifi, Zap, ThumbsUp, ThumbsDown, Languages } from 'lucide-react';
...
const USDA_KEYWORDS = [
  'housing', 'home', 'mortgage', 'rent', 'rental', 'loan', 'grant', 'repair', 'rehabilitation',
  'business', 'entrepreneur', 'small business', 'cooperative', 'startup', 'financing',
  'broadband', 'internet', 'telecommunications', 'connectivity', 'fiber', 'high-speed',
  'energy', 'renewable', 'solar', 'efficiency', 'electric', 'REAP', 'wind', 'biomass',
  'water', 'waste', 'wastewater', 'disposal', 'sanitation', 'drinking water',
  'community', 'facilities', 'health', 'healthcare', 'hospital', 'clinic', 'school',
  'rural', 'farm', 'agriculture', 'farming', 'producer', 'value-added',
  'development', 'infrastructure', 'funding', 'finance', 'investment',
  'telemedicine', 'distance learning', 'education', 'training',
  'multi-family', 'apartment', 'elderly', 'senior', 'disability',
  'microloan', 'microenterprise', 'technical assistance',
  'usda', 'program', 'assistance', 'support', 'help', 'apply', 'eligible', 'qualify'
];
...
function App() {
  ...
  const searchPrograms = async (keywords: string[]): Promise<Program[]> => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('usda_programs')
        .select('title, description, url, category, eligibility, benefits, application_process, contact_info, funding_range, program_code')
        .or(searchConditions)
        .limit(5);
  ...
  const generateConversationalResponse = (
    ...
    Object.entries(categoryGroups).forEach(([category, categoryPrograms]) => {
      ...
      categoryPrograms.forEach((program) => {
        ...
        if (program.description) {
          ...
          response += `  ${desc}${program.description.length > 120 ? '...' : ''}\n`;
        }

        if (program.benefits) {
          response += `  💰 Benefits: ${program.benefits.substring(0, 100)}${program.benefits.length > 100 ? '...' : ''}\n`;
        }

        if (program.eligibility) {
          response += `  ✓ Eligibility: ${program.eligibility.substring(0, 100)}${program.eligibility.length > 100 ? '...' : ''}\n`;
        }
    ...
    response += '💡 Would you like more details about any of these programs? I can help with eligibility requirements, application process, or contact information!';
    return response;
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseContent = generateConversationalResponse(queryText, keywords, programs);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-400 to-blue-100 flex items-center justify-center p-4">
      ...
      <div className={`fixed inset-0 flex items-center justify-center pointer-events-none transition-all duration-300 ${isOpen ? 'opacity-0 blur-sm' : 'opacity-100 blur-0'}`}>
        <div className="relative max-w-2xl px-4 -mt-32">
          <div className="text-center backdrop-blur-sm bg-white/30 rounded-3xl p-8 shadow-2xl">
            <h2 className="text-4xl md:text-5xl font-bold bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 bg-clip-text text-transparent mb-3 drop-shadow-lg">
          ...
          {/* Simple curved arrow */}
          <svg
            className="absolute left-1/2 ml-12 top-32 w-48 h-56 drop-shadow-lg pointer-events-none"
            viewBox="0 0 200 250"
            fill="none"
            strokeLinecap="round"
          ...
          >
            {/* Clean curved path */}
            <path
              d="M 80 20 Q 60 60, 80 100 Q 100 140, 120 180 Q 130 200, 150 210"
              stroke="#475569"
              strokeWidth="2.5"
              opacity="0.7"
            />
            {/* Arrow head */}
            <path d="M 150 210 L 142 204" stroke="#475569" strokeWidth="2.5" opacity="0.7" />
            <path d="M 150 210 L 145 218" stroke="#475569" strokeWidth="2.5" opacity="0.7" />
          </svg>
        </div>
      </div>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-4 right-4 w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center shadow-lg hover:bg-slate-700 transition-colors z-10 animate-bounce"
        >
                ...
                <div className="absolute right-0 mt-2 w-32 bg-white rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${
                      language === 'es' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                    }`}
                  >
                    Español
                  </button>
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 rounded-b-lg ${
                      language === 'ar' ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'
                    }`}
                  >
                    العربية
                  </button>
                ...
                >
                  {message.attachment && (
                    <div className={`mb-2 p-2 rounded ${
                      message.role === 'user' ? 'bg-green-600' : 'bg-gray-100'
                    }`}>
                      <div className="flex items-center gap-2">
                        <Paperclip size={16} />
          ...
          <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
            <div className="flex gap-2 overflow-x-auto">
              ...
              >
                ...
                <span className="text-gray-700">{translations[language].housing}</span>
              </button>
              <button
                onClick={() => setInput('What business programs are available?')}
                className="flex items-center gap-2 px-3 py-2 bg-white rounded-full border border-gray-200 hover:border-blue-500 hover:bg-blue-50 transition-colors whitespace-nowrap text-sm"
              >
                <Briefcase size={16} className="text-red-800" />
                <span className="text-gray-700">{translations[language].business}</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-gray-700">{translations[language].broadband}</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-gray-700">{translations[language].energy}</span>
              </button>
            </div>
          </div>
            ...
            {attachedFile && (
              <div className="mb-2 p-2 bg-blue-50 rounded-lg flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-medium text-gray-800">{attachedFile.name}</p>
                    <p className="text-xs text-gray-600">{(attachedFile.size / 1024).toFixed(2)} KB</p>
                  </div>
                </div>
                <button
                  type="button"
